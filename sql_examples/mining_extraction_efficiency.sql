...
CREATE FOLDER IF NOT EXISTS RetailDB.Mining.Gold;

-------------------------------------------------------------------------------
-- 1. BRONZE LAYER
-------------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS RetailDB.Mining.Bronze.HaulCycles (
    CycleID INT,
    TruckID INT,
    LoadTime TIMESTAMP,
    DumpTime TIMESTAMP,
...
INSERT INTO RetailDB.Mining.Bronze.HaulCycles VALUES
...
CREATE OR REPLACE VIEW RetailDB.Mining.Silver.ProductionMetrics AS
SELECT 
    CycleID,
    TruckID,
    PitLocation,
...
FROM RetailDB.Mining.Bronze.HaulCycles;

-------------------------------------------------------------------------------
-- 3. GOLD LAYER
-------------------------------------------------------------------------------

CREATE OR REPLACE VIEW RetailDB.Mining.Gold.PitPerformance AS
SELECT 
    PitLocation,
    COUNT(*) AS HaulCount,
...
FROM RetailDB.Mining.Silver.ProductionMetrics
...
/*
PROMPT:
"Which PitLocation produces the highest TotalMetal in RetailDB.Mining.Gold.PitPerformance?"
*/
